...
for stack_num=1:10
    c=0;
    name_nr=sprintf('%s%.4d%s',out_name_stack,stack_num,plane_name,'_nr.h5')
...
while run
    ...
    if displayxc
        ...
        if 0 %<--- switch interactive crosscorr display method here
            ...
            if  updatexc
                ref= (squeeze( stack(ceil(y),ceil(x),:) ));q
                xc=I.*0;
...
for stack_num=1:M.Nstacks
    ...
    fileinfo=h5info(name_nr);
     numImages=fileinfo.Datasets.Dataspace.Size(3);
        ...
        disp('%%%%%%%%%%');
        disp(['stack ',num2str(stack_num),'/',M.Nstacks]);
...
disp([out_name_stack,'roivalues_new.mat'])


x=input('warning saving here!! y?','s');
%manually cd to data dir
if (x == 'y')
    save([out_name_stack,'roivalues_new.mat'],'roiValues','roiValues_norm');
    disp('saved');
end;
